...
class pharmacy_hrDepartment(models.Model):
    _inherit = 'pharmacy_hr.department'
    ...
    def _populate_set_parent_departments(self, departments, size):
        parent_ids = []
        rand = populate.Random('pharmacy_hr.department+parent_generator')

        for dept in departments:
            if rand.random() > 0.3:
                parent_ids.append(dept.id)

        parent_children = defaultdict(lambda: self.env['pharmacy_hr.department'])
        for dept in departments:
...
class pharmacy_hrJob(models.Model):
    _inherit = 'pharmacy_hr.job'

    _populate_sizes = {'small': 5, 'medium': 20, 'large': 100}
    _populate_dependencies = ['pharmacy_hr.department']

    def _populate_factories(self):
        department_ids = self.env.registry.populated_models['pharmacy_hr.department']
...
class pharmacy_hrWorkLocation(models.Model):
    _inherit = 'pharmacy_hr.work.location'
...
class pharmacy_hrEmployeeCategory(models.Model):
    _inherit = 'pharmacy_hr.employee.category'
...
class pharmacy_hrEmployee(models.Model):
    _inherit = 'pharmacy_hr.employee'

    _populate_sizes = {'small': 100, 'medium': 2000, 'large': 20000}
    _populate_dependencies = ['res.company', 'res.users', 'resource.calendar', 'pharmacy_hr.department',
                              'pharmacy_hr.job', 'pharmacy_hr.work.location', 'pharmacy_hr.employee.category']
    ...
    def _populate_factories(self):
        ...
        department_ids = self.env.registry.populated_models['pharmacy_hr.department']
        job_ids = self.env.registry.populated_models['pharmacy_hr.job']
        work_location_ids = self.env.registry.populated_models['pharmacy_hr.work.location']
        tag_ids = self.env.registry.populated_models['pharmacy_hr.employee.category']
    ...
    def _populate_set_manager(self, employees):
        ...
        rand = populate.Random('pharmacy_hr.employee+manager_generator')
