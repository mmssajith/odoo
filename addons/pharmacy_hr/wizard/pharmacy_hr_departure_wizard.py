...
class pharmacy_hrDepartureWizard(models.TransientModel):
    _name = 'pharmacy_hr.departure.wizard'
    _description = 'Departure Wizard'

    def _get_employee_departure_date(self):
        return self.env['pharmacy_hr.employee'].browse(self.env.context['active_id']).departure_date
    ...
    departure_reason_id = fields.Many2one("pharmacy_hr.departure.reason", default=lambda self: self.env['pharmacy_hr.departure.reason'].search([], limit=1), required=True)
    ...
    employee_id = fields.Many2one(
        'pharmacy_hr.employee', string='Employee', required=True,
