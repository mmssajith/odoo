...
class MailActivitySchedule(models.TransientModel):
    ...
    department_id = fields.Many2one('pharmacy_hr.department', compute='_compute_department_id')

    @api.depends('department_id')
    def _compute_plan_available_ids(self):
        todo = self.filtered(lambda s: s.res_model == 'pharmacy_hr.employee')
    ...
    def _compute_department_id(self):
        for wizard in self:
            if wizard.res_model == 'pharmacy_hr.employee':
