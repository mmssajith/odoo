...
QUnit.test("many2one_avatar_employee widget in list view", async function (assert) {
    ...
    const [employeeId_1, employeeId_2] = pyEnv["pharmacy_hr.employee.public"].create([
    ...
    // assert.verifySteps([`read pharmacy_hr.employee.public ${employeeId_1}`]);
    ...
    // assert.verifySteps([`read pharmacy_hr.employee.public ${employeeId_2}`]);
    // assert.strictEqual(
...
QUnit.test("many2one_avatar_employee widget in kanban view", async function (assert) {
    ...
    const employeeId = pyEnv["pharmacy_hr.employee.public"].create({
        user_id: userId,
    ...
    assert.strictEqual(
        ...
        `/web/image/pharmacy_hr.employee.public/${employeeId}/avatar_128`
    );
});

QUnit.test("many2one_avatar_employee with pharmacy_hr group widget in kanban view", async function (assert) {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({});
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const employeeId = pyEnv["pharmacy_hr.employee.public"].create({
    ...
    assert.strictEqual(
        document.querySelector(".o_m2o_avatar > img").getAttribute("data-src"),
        `/web/image/pharmacy_hr.employee/${employeeId}/avatar_128`
    );
});
    ...
    const views = {
        "m2x.avatar.employee,false,kanban": `<kanban>
                <templates>
                    <t t-name="kanban-box">
                        <div>
                            <field name="employee_id" widget="many2one_avatar_employee" options="{'relation': 'pharmacy_hr.employee.public'}"/>
                        </div>
                    </t>
                </templates>
            </kanban>`,
...
QUnit.test("many2one_avatar_employee without pharmacy_hr.group_pharmacy_hr_user", async function (assert) {
    assert.expect(2);

    setupViewRegistries();
    serviceRegistry.add(
        "user",
        makeFakeUserService((group) => group !== "pharmacy_hr.group_pharmacy_hr_user"),
        { force: true }
    );
    ...
    await makeView({
        ...
        serverData: {
            models: {
                "m2x.avatar.employee": {
                    fields: {
                        employee_id: {
                            string: "employee", type: "one2many", relation: "pharmacy_hr.employee"
                        },
                    },
                    records: [],
                },
            ...
            views: {
                "pharmacy_hr.employee.public,false,list": `<tree><field name="display_name"/></tree>`,
                "pharmacy_hr.employee.public,false,search": `<search></search>`,
                "pharmacy_hr.employee,false,list": `<tree><field name="display_name"/></tree>`,
                "pharmacy_hr.employee,false,search": `<search></search>`,
            }
        },
        ...
        mockRPC: function (_, { model, method }) {
            if (method === "name_search" || method === "web_search_read") {
                assert.strictEqual(model, "pharmacy_hr.employee.public");
            }
        }
    });
...
QUnit.test(
    ...
    async function (assert) {
        const pyEnv = await startServer();
        const employeeId = pyEnv["pharmacy_hr.employee.public"].create({ name: "Mario" });
...
QUnit.test("many2many_avatar_employee widget in form view", async function (assert) {
    ...
    assert.strictEqual(
        ...
        `/web/image/pharmacy_hr.employee.public/${employeeId_1}/avatar_128`
    );
...
QUnit.test("many2many_avatar_employee with pharmacy_hr group widget in form view", async function (assert) {
    const pyEnv = await startServer();
    const [partnerId_1, partnerId_2] = pyEnv["res.partner"].create([{}, {}]);
    const [userId_1, userId_2] = pyEnv["res.users"].create([
        { partner_id: partnerId_1 },
    ...
    // if the user doesn't have access to pharmacy_hr.group_pharmacy_hr_user, the employee field should show the public employee
    serviceRegistry.add(
        "user",
        makeFakeUserService(() => true),
        { force: true }
    ...
    assert.strictEqual(
        document
            .querySelector(".o_field_many2many_avatar_employee .o_tag img")
            .getAttribute("data-src"),
        `/web/image/pharmacy_hr.employee/${employeeId_1}/avatar_128`
    );
...
QUnit.test("many2many_avatar_employee widget in kanban view", async function (assert) {
    ...
    assert.strictEqual(
        document
            ...
            .getAttribute("data-src"),
        `/web/image/pharmacy_hr.employee.public/${employeeId_2}/avatar_128`
    );
    assert.strictEqual(
        document
            .querySelectorAll(
