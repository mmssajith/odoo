...
export class DepartmentChart extends Component {
    ...
    async fetchHierarchy(departmentId) {
        this.state.hierarchy = await this.orm.call("pharmacy_hr.department", "get_department_hierarchy", [
            departmentId,
        ]);
    }

    openDepartmentEmployees(departmentId) {
        this.action.doAction("pharmacy_hr.act_employee_from_department", {
            additionalContext: {
...
DepartmentChart.template = "pharmacy_hr.departmentChart";
...
registry.category("view_widgets").add("pharmacy_hr_department_chart", departmentChart);
