...
class TestSelfAccessProfile(Testpharmacy_hrCommon):
    ...
    def test_access_my_profile(self):
        ...
        self.env['pharmacy_hr.employee'].create({
            'name': 'James',
            'user_id': james.id,
        })
        view = self.env.ref('pharmacy_hr.res_users_view_form_profile')
    ...
    def test_readonly_fields(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('pharmacy_hr.pharmacy_hr_employee_self_edit', False)
        james = new_test_user(self.env, login='hel', groups='base.group_user', name='Simple employee', email='ric@example.com')
        james = james.with_user(james)
        self.env['pharmacy_hr.employee'].create({
            'name': 'James',
...
class TestSelfAccessRights(Testpharmacy_hrCommon):
    ...
    def setUpClass(cls):
        ...
        cls.richard_emp = cls.env['pharmacy_hr.employee'].create({
            'name': 'Richard',
        ...
        cls.hubert_emp = cls.env['pharmacy_hr.employee'].create({
            'name': 'Hubert',
            'user_id': cls.hubert.id,
        })

        cls.protected_fields_emp = OrderedDict([(k, v) for k, v in cls.env['pharmacy_hr.employee']._fields.items() if v.groups == 'pharmacy_hr.group_pharmacy_hr_user'])
        # Compute fields and id field are always readable by everyone
        cls.read_protected_fields_emp = OrderedDict([(k, v) for k, v in cls.env['pharmacy_hr.employee']._fields.items() if not v.compute and k != 'id'])
        cls.self_protected_fields_user = OrderedDict([
            (k, v)
            for k, v in cls.env['res.users']._fields.items()
            if v.groups == 'pharmacy_hr.group_pharmacy_hr_user' and k in cls.env['res.users'].SELF_READABLE_FIELDS
        ])

    # Read pharmacy_hr.employee #
    ...
    def testWriteSelfUserEmployee(self):
        self.env['ir.config_parameter'].set_param('pharmacy_hr.pharmacy_hr_employee_self_edit', True)
        for f, v in self.self_protected_fields_user.items():
            val = None
    ...
    def testWriteSelfUserPreferencesEmployee(self):
        # self should always be able to update non pharmacy_hr.employee fields if
        # they are in SELF_READABLE_FIELDS
        self.env['ir.config_parameter'].set_param('pharmacy_hr.pharmacy_hr_employee_self_edit', False)
        # should not raise
        vals = [
            {'tz': "Australia/ACT"},
    ...
    def test_onchange_readable_fields_with_no_access(self):
        ...
        with Form(self.richard.with_user(self.richard), view='pharmacy_hr.res_users_view_form_profile') as form:
