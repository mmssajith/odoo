...
from .common import Testpharmacy_hrCommon


class TestResource(Testpharmacy_hrCommon):

    @classmethod
    def setUpClass(cls):
        super(TestResource, cls).setUpClass()
        cls.calendar_40h = cls.env['resource.calendar'].create({'name': 'Default calendar'})
        cls.employee_niv = cls.env['pharmacy_hr.employee'].create({
        ...
        cls.env.cr.execute("UPDATE pharmacy_hr_employee SET create_date=%s WHERE id=%s",
