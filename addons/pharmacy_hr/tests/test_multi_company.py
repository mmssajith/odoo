...
class TestMultiCompany(Testpharmacy_hrCommon):
    ...
    def setUpClass(cls):
        ...
        cls.res_users_pharmacy_hr_officer.company_ids = [
            (4, cls.company_1.id),
            (4, cls.company_2.id),
        ]
        cls.res_users_pharmacy_hr_officer.company_id = cls.company_1.id
    ...
    def test_multi_company_report(self):
        content, _ = self.env['ir.actions.report'].with_user(self.res_users_pharmacy_hr_officer).with_context(
            allowed_company_ids=[self.company_1.id, self.company_2.id]
        )._render_qweb_pdf('pharmacy_hr.pharmacy_hr_employee_print_badge', res_ids=self.employees.ids)
    ...
    def test_single_company_report(self):
        with self.assertRaises(QWebException):  # CacheMiss followed by AccessError
            self.env['ir.actions.report'].with_user(self.res_users_pharmacy_hr_officer).with_company(
                self.company_1
            )._render_qweb_pdf('pharmacy_hr.pharmacy_hr_employee_print_badge', res_ids=self.employees.ids)
