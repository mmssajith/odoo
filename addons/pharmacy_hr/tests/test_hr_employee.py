...
class Testpharmacy_hrEmployee(Testpharmacy_hrCommon):
    ...
    def setUp(self):
        ...
        self.employee_without_image = self.env['pharmacy_hr.employee'].create({
    ...
    def test_employee_resource(self):
        _tz = 'Pacific/Apia'
        self.res_users_pharmacy_hr_officer.company_id.resource_calendar_id.tz = _tz
        Employee = self.env['pharmacy_hr.employee'].with_user(self.res_users_pharmacy_hr_officer)
    ...
    def test_employee_from_user(self):
        ...
        self.res_users_pharmacy_hr_officer.company_id.resource_calendar_id.tz = _tz
        self.res_users_pharmacy_hr_officer.tz = _tz2
        Employee = self.env['pharmacy_hr.employee'].with_user(self.res_users_pharmacy_hr_officer)
        employee_form = Form(Employee)
        employee_form.name = 'Raoul Grosbedon'
        employee_form.work_email = 'raoul@example.com'
        employee_form.user_id = self.res_users_pharmacy_hr_officer
        employee = employee_form.save()
        self.assertEqual(employee.name, 'Raoul Grosbedon')
        self.assertEqual(employee.work_email, self.res_users_pharmacy_hr_officer.email)
        self.assertEqual(employee.tz, self.res_users_pharmacy_hr_officer.tz)

    def test_employee_from_user_tz_no_reset(self):
        _tz = 'Pacific/Apia'
        self.res_users_pharmacy_hr_officer.tz = False
        Employee = self.env['pharmacy_hr.employee'].with_user(self.res_users_pharmacy_hr_officer)
        employee_form = Form(Employee)
        employee_form.name = 'Raoul Grosbedon'
        employee_form.work_email = 'raoul@example.com'
    ...
    def test_employee_member_of_department(self):
        dept, dept_sub, dept_sub_sub, dept_other, dept_parent = self.env['pharmacy_hr.department'].create([
            {
                'name': 'main',
            },
            {
        ...
        emp, emp_sub, emp_sub_sub, emp_other, emp_parent = self.env['pharmacy_hr.employee'].with_user(self.res_users_pharmacy_hr_officer).create([
            {
                'name': 'employee',
        ...
        self.res_users_pharmacy_hr_officer.employee_id = emp
    ...
    def test_employee_create_from_user(self):
        employee = self.env['pharmacy_hr.employee'].create({
    ...
    def test_employee_update_work_contact_id(self):
        ...
        employee_A, employee_B = self.env['pharmacy_hr.employee'].create([
            {
                'name': 'Employee A',
                'user_id': user.id,
    ...
    def test_change_user_on_employee(self):
        ...
        test_employee = self.env['pharmacy_hr.employee'].create({
