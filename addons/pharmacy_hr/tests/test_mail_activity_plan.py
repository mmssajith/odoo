...
class ActivitySchedulepharmacy_hrCase(ActivityScheduleCase):
    ...
    def setUpClass(cls):
        ...
        cls.plan_onboarding = cls.env['mail.activity.plan'].create({
            ...
            'res_model': 'pharmacy_hr.employee',
            'template_ids': [
                Command.create({
        ...
        cls.user_manager = mail_new_test_user(
            ...
            groups='base.group_user,pharmacy_hr.group_pharmacy_hr_manager',
            login='test_manager',
            name='Test Manager',
        )
        ...
        cls.employees = cls.env['pharmacy_hr.employee'].create([
            {
                'name': user.name,
                'user_id': user.id,
        ...
        cls.department_a = cls.env['pharmacy_hr.department'].create({
            'name': 'Test Department A',
            'member_ids': [Command.link(employee.id) for employee in cls.employees - cls.employee_dep_b],
        })
        cls.department_b = cls.env['pharmacy_hr.department'].create({
...
class TestActivitySchedule(ActivitySchedulepharmacy_hrCase):
    ...
    def test_res_model_compatibility(self):
        ...
        of employee if pharmacy_hr plan specific features are used. """
        ...
        self.plan_onboarding.res_model = 'res.partner'
        self.plan_onboarding.res_model = 'pharmacy_hr.employee'
