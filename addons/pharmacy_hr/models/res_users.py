...
pharmacy_hr_READABLE_FIELDS = [
    'active',
    'child_ids',
    'employee_id',
    'employee_ids',
    'employee_parent_id',
    'pharmacy_hr_presence_state',
    'last_activity',
    'last_activity_time',
    'can_edit',
    'is_system',
...
class User(models.Model):
    ...
    # note: a user can only be linked to one employee per company (see sql constraint in ´pharmacy_hr.employee´)
    employee_ids = fields.One2many('pharmacy_hr.employee', 'user_id', string='Related employee', domain=_employee_ids_domain)
    employee_id = fields.Many2one('pharmacy_hr.employee', string="Company employee",
    ...
    category_ids = fields.Many2many(related='employee_id.category_ids', string="Pharmacy Employee Tags", readonly=False, related_sudo=False)
    ...
    pharmacy_hr_presence_state = fields.Selection(related='employee_id.pharmacy_hr_presence_state')
    ...
    create_employee_id = fields.Many2one('pharmacy_hr.employee', store=False, copy=False, string="Technical field, bind user to this employee on create")
    ...
    def _compute_can_edit(self):
        can_edit = self.env['ir.config_parameter'].sudo().get_param('pharmacy_hr.pharmacy_hr_employee_self_edit') or self.env.user.has_group('pharmacy_hr.group_pharmacy_hr_user')
        for user in self:
    ...
    def SELF_READABLE_FIELDS(self):
        return super().SELF_READABLE_FIELDS + pharmacy_hr_READABLE_FIELDS + pharmacy_hr_WRITABLE_FIELDS

    @property
    def SELF_WRITEABLE_FIELDS(self):
        return super().SELF_WRITEABLE_FIELDS + pharmacy_hr_WRITABLE_FIELDS
    ...
    def get_views(self, views, options=None):
        ...
        # in `get_view()`.
        profile_view = self.env.ref("pharmacy_hr.res_users_view_form_profile")
    ...
    def create(self, vals_list):
        ...
        for user, vals in zip(res, vals_list):
            ...
            if vals.get('create_employee_id'):
                self.env['pharmacy_hr.employee'].browse(vals.get('create_employee_id')).user_id = user
            else:
                employee_create_vals.append(dict(
                    name=user.name,
                    company_id=user.env.company.id,
                    **self.env['pharmacy_hr.employee']._sync_user(user)
                ))
        if employee_create_vals:
            self.env['pharmacy_hr.employee'].with_context(clean_context(self.env.context)).create(employee_create_vals)
        return res
    ...
    def write(self, vals):
        """
        Syncpharmacy_hronize user and its related employee
        ...
        pharmacy_hr_fields = {
            ...
            if field.related_field and field.related_field.model_name == 'pharmacy_hr.employee' and field_name in vals
        }
        can_edit_self = self.env['ir.config_parameter'].sudo().get_param('pharmacy_hr.pharmacy_hr_employee_self_edit') or self.env.user.has_group('pharmacy_hr.group_pharmacy_hr_user')
        if pharmacy_hr_fields and not can_edit_self:
            # Raise meaningful error message
            raise AccessError(_("You are only allowed to update your preferences. Please contact a pharmacy_hr officer to update other information."))

        employee_domain = [
            *self.env['pharmacy_hr.employee']._check_company_domain(self.env.company),
            ('user_id', 'in', self.ids),
        ]
        if pharmacy_hr_fields:
            employees = self.env['pharmacy_hr.employee'].sudo().search(employee_domain)
            get_field = self.env['ir.model.fields']._get
            field_names = Markup().join([
                 Markup("<li>%s</li>") % get_field("res.users", fname).field_description for fname in pharmacy_hr_fields
            ])
        ...
        if employee_values:
            ...
            if 'image_1920' in vals:
                without_image = self.env['pharmacy_hr.employee'].sudo().search(employee_domain + [('image_1920', '=', False)])
                with_image = self.env['pharmacy_hr.employee'].sudo().search(employee_domain + [('image_1920', '!=', False)])
    ...
    def action_get(self):
        if self.env.user.employee_id:
            return self.env['ir.actions.act_window']._for_xml_id('pharmacy_hr.res_users_action_my')
    ...
    def _compute_company_employee(self):
        employee_per_user = {
            ...
            for employee in self.env['pharmacy_hr.employee'].search([('user_id', 'in', self.ids), ('company_id', '=', self.env.company.id)])
        }
        for user in self:
    ...
    def action_create_employee(self):
        self.ensure_one()
        self.env['pharmacy_hr.employee'].create(dict(
            name=self.name,
            company_id=self.env.company.id,
            **self.env['pharmacy_hr.employee']._sync_user(self)
        ))

    def action_open_employees(self):
        self.ensure_one()
        employees = self.employee_ids
        model = 'pharmacy_hr.employee' if self.user_has_groups('pharmacy_hr.group_pharmacy_hr_user') else 'pharmacy_hr.employee.public'
        if len(employees) > 1:
            return {
