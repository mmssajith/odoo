...
class pharmacy_hrEmployeePrivate(models.Model):
    """
    NB: Any field only available on the model pharmacy_hr.employee (i.e. not on the
    pharmacy_hr.employee.public model) should have `groups="pharmacy_hr.group_pharmacy_hr_user"` on its
    definition to avoid being prefetched when the user hasn't access to the
    pharmacy_hr.employee model. Indeed, the prefetch loads the data for all the fields
    that are available according to the group defined on them.
    """
    _name = "pharmacy_hr.employee"
    _description = "Employee"
    _order = 'name'
    _inherit = ['pharmacy_hr.employee.base', 'mail.thread.main.attachment', 'mail.activity.mixin', 'resource.mixin', 'avatar.mixin']
    ...
    # private info
    private_street = fields.Char(string="Private Street", groups="pharmacy_hr.group_pharmacy_hr_user")
    private_street2 = fields.Char(string="Private Street2", groups="pharmacy_hr.group_pharmacy_hr_user")
    private_city = fields.Char(string="Private City", groups="pharmacy_hr.group_pharmacy_hr_user")
    private_state_id = fields.Many2one(
        "res.country.state", string="Private State",
        domain="[('country_id', '=?', private_country_id)]",
        groups="pharmacy_hr.group_pharmacy_hr_user")
    private_zip = fields.Char(string="Private Zip", groups="pharmacy_hr.group_pharmacy_hr_user")
    private_country_id = fields.Many2one("res.country", string="Private Country", groups="pharmacy_hr.group_pharmacy_hr_user")
    private_phone = fields.Char(string="Private Phone", groups="pharmacy_hr.group_pharmacy_hr_user")
    private_email = fields.Char(string="Private Email", groups="pharmacy_hr.group_pharmacy_hr_user")
    lang = fields.Selection(selection=_lang_get, string="Lang", groups="pharmacy_hr.group_pharmacy_hr_user")
    country_id = fields.Many2one(
        'res.country', 'Nationality (Country)', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    gender = fields.Selection([
        ('male', 'Male'),
        ('female', 'Female'),
        ('other', 'Other')
    ], groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    marital = fields.Selection([
        ...
        ('widower', 'Widower'),
        ('divorced', 'Divorced')
    ], string='Marital Status', groups="pharmacy_hr.group_pharmacy_hr_user", default='single', tracking=True)
    spouse_complete_name = fields.Char(string="Spouse Complete Name", groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    spouse_birthdate = fields.Date(string="Spouse Birthdate", groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    children = fields.Integer(string='Number of Dependent Children', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    place_of_birth = fields.Char('Place of Birth', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    country_of_birth = fields.Many2one('res.country', string="Country of Birth", groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    birthday = fields.Date('Date of Birth', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    ssnid = fields.Char('SSN No', help='Social Security Number', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    sinid = fields.Char('SIN No', help='Social Insurance Number', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    identification_id = fields.Char(string='Identification No', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    passport_id = fields.Char('Passport No', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    ...
    permit_no = fields.Char('Work Permit No', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    visa_no = fields.Char('Visa No', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    visa_expire = fields.Date('Visa Expiration Date', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    work_permit_expiration_date = fields.Date('Work Permit Expiration Date', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    has_work_permit = fields.Binary(string="Work Permit", groups="pharmacy_hr.group_pharmacy_hr_user")
    work_permit_scheduled_activity = fields.Boolean(default=False, groups="pharmacy_hr.group_pharmacy_hr_user")
    work_permit_name = fields.Char('work_permit_name', compute='_compute_work_permit_name')
    additional_note = fields.Text(string='Additional Note', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    certificate = fields.Selection([
        ...
        ('doctor', 'Doctor'),
        ('other', 'Other'),
    ], 'Certificate Level', default='other', groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    study_field = fields.Char("Field of Study", groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    study_school = fields.Char("School", groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    emergency_contact = fields.Char("Contact Name", groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    emergency_phone = fields.Char("Contact Phone", groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    km_home_work = fields.Integer(string="Home-Work Distance", groups="pharmacy_hr.group_pharmacy_hr_user", tracking=True)
    employee_type = fields.Selection([
        ...
        ], string='Employee Type', default='employee', required=True, groups="pharmacy_hr.group_pharmacy_hr_user",
    ...
    # employee in company
    child_ids = fields.One2many('pharmacy_hr.employee', 'parent_id', string='Direct subordinates')
    category_ids = fields.Many2many(
        'pharmacy_hr.employee.category', 'employee_category_rel',
        'emp_id', 'category_id', groups="pharmacy_hr.group_pharmacy_hr_user",
        string='Tags')
    # misc
    notes = fields.Text('Notes', groups="pharmacy_hr.group_pharmacy_hr_user")
    color = fields.Integer('Color Index', default=0)
    barcode = fields.Char(string="Badge ID", help="ID used for employee identification.", groups="pharmacy_hr.group_pharmacy_hr_user", copy=False)
    pin = fields.Char(string="PIN", groups="pharmacy_hr.group_pharmacy_hr_user", copy=False,
        help="PIN used to Check In/Out in the Kiosk Mode of the Attendance application (if enabled in Configuration) and to change the cashier in the Point of Sale application.")
    departure_reason_id = fields.Many2one("pharmacy_hr.departure.reason", string="Departure Reason", groups="pharmacy_hr.group_pharmacy_hr_user",
                                          copy=False, tracking=True, ondelete='restrict')
    departure_description = fields.Html(string="Additional Information", groups="pharmacy_hr.group_pharmacy_hr_user", copy=False)
    departure_date = fields.Date(string="Departure Date", groups="pharmacy_hr.group_pharmacy_hr_user", copy=False, tracking=True)
    message_main_attachment_id = fields.Many2one(groups="pharmacy_hr.group_pharmacy_hr_user")
    id_card = fields.Binary(string="ID Card Copy", groups="pharmacy_hr.group_pharmacy_hr_user")
    driving_license = fields.Binary(string="Driving License", groups="pharmacy_hr.group_pharmacy_hr_user")
    private_car_plate = fields.Char(groups="pharmacy_hr.group_pharmacy_hr_user", help="If you have more than one car, just separate the plates by a space.")
    ...
    def _compute_avatar(self, avatar_field, image_field):
        ...
        super(pharmacy_hrEmployeePrivate, self.browse(employee_wo_user_or_image_ids))._compute_avatar(avatar_field, image_field)
    ...
    def action_create_user(self):
        ...
        return {
            ...
            'res_model': 'res.users',
            'view_mode': 'form',
            'view_id': self.env.ref('pharmacy_hr.view_users_simple_form').id,
            'target': 'new',
    ...
    def _compute_display_name(self):
        ...
        for employee_private, employee_public in zip(self, self.env['pharmacy_hr.employee.public'].browse(self.ids)):
    ...
    def search_fetch(self, domain, field_names, offset=0, limit=None, order=None):
        ...
        # HACK: retrieve publicly available values from pharmacy_hr.employee.public and
        ...
        public = self.env['pharmacy_hr.employee.public'].search_fetch(domain, field_names, offset, limit, order)
    ...
    def fetch(self, field_names):
        ...
        public = self.env['pharmacy_hr.employee.public'].browse(self._ids)
    ...
    def _check_private_fields(self, field_names):
        ...
        public_fields = self.env['pharmacy_hr.employee.public']._fields
    ...
    def _cron_check_work_permit_validity(self):
        ...
        employees_scheduled = self.env['pharmacy_hr.employee']
    ...
    def get_view(self, view_id=None, view_type='form', **options):
        ...
        return self.env['pharmacy_hr.employee.public'].get_view(view_id, view_type, **options)
    ...
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        """
            ...
            search on an pharmacy_hr.employee returns a pharmacy_hr.employee recordset, even if you don't have access
            to this model, as the result of _search (the ids of the public employees) is to be
            browsed on the pharmacy_hr.employee model. This can be trusted as the ids of the public
            employees exactly match the ids of the related pharmacy_hr.employee.
        """
        if self.check_access_rights('read', raise_exception=False):
            return super()._search(domain, offset, limit, order, access_rights_uid)
        try:
            ids = self.env['pharmacy_hr.employee.public']._search(domain, offset, limit, order, access_rights_uid)
        except ValueError:
            raise AccessError(_('You do not have access to this document.'))
        # the result is expected from this table, so we should link tables
        return super(pharmacy_hrEmployeePrivate, self.sudo())._search([('id', 'in', ids)], order=order)
    ...
    def get_formview_id(self, access_uid=None):
        ...
        if self_sudo.check_access_rights('read', raise_exception=False):
            return super(pharmacy_hrEmployeePrivate, self).get_formview_id(access_uid=access_uid)
        # Hardcode the form view for public employee
        return self.env.ref('pharmacy_hr.pharmacy_hr_employee_public_view_form').id

    def get_formview_action(self, access_uid=None):
        """ Override this method in order to redirect many2one towards the right model depending on access_uid """
        res = super(pharmacy_hrEmployeePrivate, self).get_formview_action(access_uid=access_uid)
        if access_uid:
            self_sudo = self.with_user(access_uid)
        else:
            self_sudo = self
        ...
        if not self_sudo.check_access_rights('read', raise_exception=False):
            res['res_model'] = 'pharmacy_hr.employee.public'
    ...
    def create(self, vals_list):
        ...
        pharmacy_hr_root_menu = self.env.ref('pharmacy_hr.menu_pharmacy_hr_root')
        for employee in employees:
            # Launch onboarding plans
            url = '/web#%s' % url_encode({
                'action': 'pharmacy_hr.plan_wizard_action',
                'active_id': employee.id,
                'active_model': 'pharmacy_hr.employee',
                'menu_id': pharmacy_hr_root_menu.id,
            })
            onboarding_notes_bodies[employee.id] = Markup(_(
                '<b>Congratulations!</b> May I recommend you to setup an <a pharmacy_href="%s">onboarding plan?</a>',
            )) % url
    ...
    def write(self, vals):
        ...
        res = super(pharmacy_hrEmployeePrivate, self).write(vals)
    ...
    def unlink(self):
        ...
        super(pharmacy_hrEmployeePrivate, self).unlink()
    ...
    def toggle_active(self):
        res = super(pharmacy_hrEmployeePrivate, self).toggle_active()
        ...
        if archived_employees:
            ...
            employees = self.env['pharmacy_hr.employee'].search(expression.OR(employee_domain + user_domain))
            for employee in employees:
        ...
        if len(self) == 1 and not self.active and not self.env.context.get('no_wizard', False):
            return {
                ...
                'res_model': 'pharmacy_hr.departure.wizard',
                'view_mode': 'form',
                'target': 'new',
    ...
    def get_import_templates(self):
        return [{
            ...
            'template': '/pharmacy_hr/static/xls/pharmacy_hr_employee.xls'
        }]
