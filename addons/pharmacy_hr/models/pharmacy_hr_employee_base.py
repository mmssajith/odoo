...
class pharmacy_hrEmployeeBase(models.AbstractModel):
    _name = "pharmacy_hr.employee.base"
    ...
    department_id = fields.Many2one('pharmacy_hr.department', 'Department', check_company=True)
    ...
    job_id = fields.Many2one('pharmacy_hr.job', 'Job Position', check_company=True)
    ...
    work_location_id = fields.Many2one('pharmacy_hr.work.location', 'Work Location')
    user_id = fields.Many2one('res.users')
    resource_id = fields.Many2one('resource.resource')
    resource_calendar_id = fields.Many2one('resource.calendar', check_company=True)
    parent_id = fields.Many2one('pharmacy_hr.employee', 'Manager', compute="_compute_parent_id", store=True, readonly=False,
        check_company=True)
    coach_id = fields.Many2one(
        'pharmacy_hr.employee', 'Coach', compute='_compute_coach', store=True, readonly=False,
        check_company=True,
    ...
    show_pharmacy_hr_icon_display = fields.Boolean(compute='_compute_presence_icon')
    ...
    def _search_newly_hired(self, operator, value):
        new_hire_field = self._get_new_hire_field()
        new_hires = self.env['pharmacy_hr.employee'].sudo().search([
    ...
    def _compute_part_of_department(self):
        ...
        if not active_department:
            ...
            def get_all_children(department):
                ...
                if not children:
                    return self.env['pharmacy_hr.department']
    ...
    def _compute_presence_state(self):
        ...
        presence criterions. e.g. pharmacy_hr_attendance, pharmacy_hr_holidays
        """
        # Check on login
        check_login = literal_eval(self.env['ir.config_parameter'].sudo().get_param('pharmacy_hr.pharmacy_hr_presence_control_login', 'False'))
        ...
        for employee in self:
            ...
            if check_login:
                if employee.user_id.im_status in ['online', 'leave_online']:
                    ...
                    state = 'absent'
            employee.pharmacy_hr_presence_state = state
    ...
    @api.depends('resource_calendar_id', 'pharmacy_hr_presence_state')
    def _compute_presence_icon(self):
        ...
        """
        working_now_list = self.filtered(lambda e: e.pharmacy_hr_presence_state == 'present')._get_employee_working_now()
        for employee in self:
            show_icon = True
            if employee.pharmacy_hr_presence_state == 'present':
                if employee.id in working_now_list:
                    icon = 'presence_present'
                else:
                    icon = 'presence_absent_active'
            elif employee.pharmacy_hr_presence_state == 'absent':
                ...
                if not employee.user_id:
                    ...
                    show_icon = False
            employee.pharmacy_hr_icon_display = icon
            employee.show_pharmacy_hr_icon_display = show_icon
