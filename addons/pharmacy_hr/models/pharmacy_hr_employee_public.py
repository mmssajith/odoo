...
class pharmacy_hrEmployeePublic(models.Model):
    _name = "pharmacy_hr.employee.public"
    _inherit = ["pharmacy_hr.employee.base"]
    _description = 'Public Employee'
    _order = 'name'
    _auto = False
    _log_access = True # Include magic fields

    # Fields coming from pharmacy_hr.employee.base
    ...
    employee_id = fields.Many2one('pharmacy_hr.employee', 'Employee', compute="_compute_employee_id", search="_search_employee_id", compute_sudo=True)
    # pharmacy_hr.employee.public specific fields
    child_ids = fields.One2many('pharmacy_hr.employee.public', 'parent_id', string='Direct subordinates', readonly=True)
    ...
    parent_id = fields.Many2one('pharmacy_hr.employee.public', 'Manager', readonly=True)
    coach_id = fields.Many2one('pharmacy_hr.employee.public', 'Coach', readonly=True)
    user_partner_id = fields.Many2one(related='user_id.partner_id', related_sudo=False, string="User's partner")

    @api.depends_context('uid')
    @api.depends('parent_id')
    def _compute_is_manager(self):
        all_reports = self.env['pharmacy_hr.employee.public'].search([('id', 'child_of', self.env.user.employee_id.id)]).ids
    ...
    def _compute_employee_id(self):
        for employee in self:
            employee.employee_id = self.env['pharmacy_hr.employee'].browse(employee.id)
