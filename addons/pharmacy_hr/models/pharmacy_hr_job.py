...
class Job(models.Model):
    ...
    employee_ids = fields.One2many('pharmacy_hr.employee', 'job_id', string='Employees', groups='base.group_user')
    ...
    department_id = fields.Many2one('pharmacy_hr.department', string='Department', check_company=True)
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)
    contract_type_id = fields.Many2one('pharmacy_hr.contract.type', string='Employment Type')
    ...
    def _compute_employees(self):
        employee_data = self.env['pharmacy_hr.employee']._read_group([('job_id', 'in', self.ids)], ['job_id'], ['__count'])
