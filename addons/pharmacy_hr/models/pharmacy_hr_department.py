...
class Department(models.Model):
    ...
    parent_id = fields.Many2one('pharmacy_hr.department', string='Parent Department', index=True, check_company=True)
    child_ids = fields.One2many('pharmacy_hr.department', 'parent_id', string='Child Departments')
    manager_id = fields.Many2one('pharmacy_hr.employee', string='Manager', tracking=True, check_company=True)
    member_ids = fields.One2many('pharmacy_hr.employee', 'department_id', string='Members', readonly=True)
    total_employee = fields.Integer(compute='_compute_total_employee', string='Total Employee')
    jobs_ids = fields.One2many('pharmacy_hr.job', 'department_id', string='Jobs')
    ...
    master_department_id = fields.Many2one(
        'pharmacy_hr.department', 'Master Department', compute='_compute_master_department_id', store=True)
    ...
    def _compute_total_employee(self):
        emp_data = self.env['pharmacy_hr.employee']._read_group([('department_id', 'in', self.ids)], ['department_id'], ['__count'])
    ...
    def create(self, vals_list):
        ...
        for department, vals in zip(departments, vals_list):
            manager = self.env['pharmacy_hr.employee'].browse(vals.get("manager_id"))
            if manager.user_id:
    ...
    def write(self, vals):
        ...
        if 'manager_id' in vals:
            ...
            if manager_id:
                manager = self.env['pharmacy_hr.employee'].browse(manager_id)
    ...
    def _update_employee_manager(self, manager_id):
        employees = self.env['pharmacy_hr.employee']
        for department in self:
            employees = employees | self.env['pharmacy_hr.employee'].search([
                ('id', '!=', manager_id),
    ...
    def get_formview_action(self, access_uid=None):
        ...
        if (not self.user_has_groups('pharmacy_hr.group_pharmacy_hr_user') and
           self.env.context.get('open_employees_kanban', False)):
            res.update({
                'name': self.name,
                'res_model': 'pharmacy_hr.employee.public',
                'view_mode': 'kanban',
    ...
    def action_plan_from_department(self):
        action = self.env['ir.actions.actions']._for_xml_id('pharmacy_hr.mail_activity_plan_action')
        action['context'] = {'default_department_id': self.id, 'search_default_department_id': self.id}
        return action

    def get_children_department_ids(self):
        return self.env['pharmacy_hr.department'].search([('id', 'child_of', self.ids)])
