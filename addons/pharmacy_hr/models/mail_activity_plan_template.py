...
class MailActivityPLanTemplate(models.Model):
    ...
    def _check_responsible_pharmacy_hr(self):
        """ Ensure that pharmacy_hr types are used only on employee model """
        for template in self.filtered(lambda tpl: tpl.plan_id.res_model != 'pharmacy_hr.employee'):
    ...
    def _determine_responsible(self, on_demand_responsible, employee):
        if self.plan_id.res_model != 'pharmacy_hr.employee' or self.responsible_type not in {'coach', 'manager', 'employee'}:
